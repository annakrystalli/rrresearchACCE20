...
knitr::include_graphics("assets/gog.jpg")
```
...
The first way we can visualise a single variable would be to plot it on a single axis.
...
We could use the same to visualise a second variable, `hp` (horsepower)
...
When we can combine these two axes, we can visualise both the values of the data on each axis as well as the relationship of the two variables:
...
Now that we've used two axes, we might want to consider another attribute to encode a thrid variable with.
...
#### attributes
appearance properties of marks
...
Lets start by mapping `mpg` and `hp` to our two axes. We do that in the second argument `mapping`, where we supply the output of function `aes()`. 
...
Any aesthetics defined in `ggplot()` will apply to all subsequent layers unless they are overriden within the individual layers.
...
Following the specification of data and aesthetics, we next need to specify which geometries (or `geoms`s in ggplot) to use to present the data. The `geom` is a critical component and describes the type of plot used.
...
Start by loading the library and reading in the data as well as `dplyr` so we have access to the pipe. Let's also narrow the data down to our variables of interest using `dplyr::select()`
...
The distribution across all our data looks quite skewed towards lower values and even shows a small dip in a certain range of `stem_diameter`. 
...
To do that let's create a vector of `growth_form` unique values, ordered according to their counts.
...
We can then specify the level order using that vector when we mutate to a factor through argument `levels` in `factor`.
...
Let's flip the axis by mapping `growth_form` to `y`, that prevents `growth_form` axes labels to overlap. 
...
However, `ggplot` and the grammar of graphics allows us to build more informative plots, by combining the information in categorical variables to present cross variable properties in distributions.
...
That's a lot clearer, and now we can see that overall the distribution of `log(stem_diameter)` across growth forms follows a broably bimodal distribution.
...
Another way we can present the distribution of a continuous variable iin a compact way and grouped according to the value of a categorical variable is to use a violin plot.
...
Violin plots show the density distribution of a continuous variable, grouped according to the values of the categorical values and presented mirrored.
...
The central liine in each box corresponds to the median.
...
Again, here we can see the bimodal distribution across all values, the two loose groups of multibole trees, single bole trees and small trees, but this time we have a must more informative plot packed in quite compactly. 

We would still need a plot per variable though. We could try and utilise `facet_grid` to combine a boxplot for each continuous variable `stem_diameter` and `height` into a single plot. Hovever, there isn't a variable in the data in the current form that could be uset to facet on. The data is instead help in 

#### Pivoting data to build plot with `facet_grid`
...
`pivot_longer()` "lengthens" data, increasing the number of rows and decreasing the number of columns. We use it to stack the values of `stem_diameter` and `height` into a column called `value` and store the original column names which define the variable each value relates to in a new column `var`.
...
Note that the number of rows is now twice the size of the original data.frame because two columns have been stacked. Those columns have also been removed.

In this format we can use variable `var` to create a facet for each variable.

#### Figure 2: Data characteristics of our raw data


Let's add that as a step in our plotting pipe
```{r}
analysis_df %>%
    tidyr::pivot_longer(cols = c(stem_diameter, height), 
                        names_to = "var",
...
We specify our model through a formula `log(stem_diameter) ~ log(height)`. This translates to log stem diameter as a function of height in which `stem_diameter` is the response variable and `height` the predictor.
...
Growth form is a categorical variable so when we include it in our regression, `lm` will fit separate coefficients for our model at every level of the factor. To include it, we added to the preictor side of our formula. If we include it as an additive effect through `+`, only the intercept will vary across factor levels. If we fit it as an interaction using `*` both the slope and intercept are allowed to vary.
...
```{r, code = readLines("analysis00.R"), eval=FALSE}
